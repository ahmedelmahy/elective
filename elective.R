...
id=vector(length = dim(d)[1]);mark=id;number=id;choice=id
  ...
  for (j in 1: length(s) ){
    ...
    if (s[j]>0) {
      if (   work[which(curz==ost)]    < as.numeric(xideal [which(curz==ost)])   ){   
        ...
        plot2$id[i]= as.character(d$ID[i])
        plot2$mark[i]=d$markss[i]
        plot2$number[i]=j
        plot2$choice[i]=ost
...
didntfill=which(!(newid %in% as.character(data$ID)))
txt3=paste('the following IDs didnot fill the form\n\n\n',
...
#report7 
plot2a=plot2[which(!is.na(as.numeric(plot2$id))),]
plot2b=plot2[which(is.na(as.numeric(plot2$id))),]
plot2a=plot2a[order(as.numeric(plot2a$id)),]
